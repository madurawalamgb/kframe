...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="text-center text-3xl font-bold text-gray-900 mb-6">Customer List</h1>
    <div class="mb-4">
...
<td class='px-4 py-2 text-sm text-gray-900'>{{ $item->address }}</td>
                    ...
                    <td class='px-4 py-2 text-sm text-gray-900'>
                        <a href="{{ route('customers.show', $item->id) }}" class="inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            View
                        </a>
                        <a href="{{ route('customers.edit', $item->id) }}" class="inline-flex items-center px-3 py-1 ml-4 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
                            Edit
                        </a>
                        <form action="{{ route('customers.destroy', $item->id) }}" method="POST" class="inline-block ml-4">
                            @csrf
                            @method('DELETE')
